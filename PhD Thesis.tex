\documentclass[12pt, twoside]{report}
\usepackage[utf8]{inputenc}

\usepackage[a4paper,top=2.5cm,bottom=2.5cm,left=3cm,right=2.5cm]{geometry}
\usepackage[portuguese,british]{babel}
\usepackage{CJKutf8}

%\usepackage[parfill]{parskip}    % Begin paragraphs with empty line instead of indent
\usepackage{graphicx}             % Use pdf, png, jpg or eps with pdflatex (eps in DVI mode)
                                  % TeX automatically converts eps --> pdf in pdflatex
\usepackage{overpic}
\usepackage{wrapfig}
\usepackage{subcaption}

\usepackage{epigraph}
\usepackage{cite}

\usepackage{amssymb}
%\usepackage{ebgaramond}

\usepackage[onehalfspacing]{setspace}
\setlength{\parskip}{0cm}
\usepackage{nameref}

% hyphenation for custom words
\PassOptionsToPackage{hyphens}{url}\usepackage[hidelinks]{hyperref}
\hyphenation{
anony-mous
anony-mous-ly
ac-ces-si-ble
ap-pli-ca-tion
tem-po-rari-ly
tem-po-rary
al-lowing
ani-mate
ani-mating
ani-ma-tion
al-ter-na-tive
editing
con-figu-ra-tion
al-ter-na-tive-ly
an-no-tate
ana-lyse
analy-ses
analy-sis
ar-ti-cle
ar-ti-cles
au-to-mati-cal-ly
available
breaking
can-di-date
cate-gori-cal
cen-tro-some
charac-te-ris-tics
clini-cal
col-labo-rating
com-bi-na-tions
com-para-tive
com-pari-son
com-puting
con-sis-tent
con-taining
cor-re-la-tion
cor-re-la-tions
de-velop-ment
de-veloping
de-pen-den-cy
di-rec-to-ry
dif-feren-tial
dif-ferent
Dockerfile
Dockerfiles
en-vi-ron-ment
epi-the-li-al-mesen-chy-mal
evalu-ate
evalu-ated
ex-panding
ex-peri-men-tal
ex-peri-ment
ex-peri-ments
figure
flexi-bili-ty
generate
generated
ge-nome
ge-no-mics
he-redi-tary
however
hy-bridi-sa-tion
image
images
in-cluding
in-creasing
in-forming
in-te-gra-ted
in-ter-pre-ta-tion
in-terac-tive
in-va-sive
in-ves-ti-ga-tion
lym-pho-ma
making
matching
medi-cal
mini-mise
mini-mised
mini-mises
modern
mo-lecu-lar
mole-cule
more-over
nema-tode
nor-mali-sa-tion
nor-mali-sed
opera-ting
opera-tive
op-por-tu-ni-ty
or-ganism
package
packages
paral-lel
per-taining
per-tur-ba-tion
per-tur-ba-tions
pheo-chromo-cyto-ma
pro-gramme
psichomics
quan-ti-fi-ca-tion
ranking
re-posi-to-ries
re-posi-to-ry
se-quencing
sen-si-tivi-ty
splicing
squa-mous
sta-tis-tics
stra-tum
TCGASpliceSeq
tracking
transcrip-tome
transcrip-tomic
transcrip-tomics
transcript
transcripts
using
value
values
visu-alisation
working
}


\usepackage[font=small,labelfont=bf]{caption}

\usepackage{listings}
\usepackage{xcolor}

\usepackage{booktabs}
\usepackage{tabularx,colortbl}	
\usepackage[alph]{parnotes}
\usepackage{multirow}
\usepackage{csquotes}

\usepackage{multicol}

%\usepackage{type1cm}
%\usepackage{lettrine}

\usepackage{relsize}

\usepackage{fontawesome}

% FUNCTIONS --------------------------------------------------------

% Formatted link, e.g. \link{https://github.com/nuno-agostinho/psichomics}{psichomics}
\newcommand*{\link}[2]{\textsmaller{\href{#1}{\nolinkurl{#2}}}}

% Small formatted link (only use in "normal" text)
%\newcommand*{\slink}[2]{\small{\link{#1}{#2}}

% Automatic link: formatted link without displaying https://, e.g.
%   \alink{docker.com} -> displays "docker.com" with a link to https://docker.com
\newcommand*{\alink}[1]{\link{https://#1}{#1}}

% Docker Hub link: formatted link to Docker Hub using only an image name, e.g.
%   \dockerlink{nunoagostinho/psichomics} -> links to hub.docker.com/r/nunoagostinho/psichomics
%   \dockerlink[_]{nginx}                 -> links to hub.docker.com/_/nginx (official Docker)
\newcommand*{\dockerlink}[2][r]{\link{https://hub.docker.com/#1/#2}{#2}}

% Fully reference a \label{}, e.g. with a previous \label{chap:methods} for Chapter 3:
%   \fullref{chap:methods} -> \textbf{chapter 3: Materials and methods}
%\newcommand*{\fullref}[2][]{\hyperref[{#2}]{\autoref*{#2}#1: \textbf{\nameref*{#2}}}}
\newcommand*{\fullref}[2][]{\shortref[#1: \textbf{\nameref*{#2}}]{#2}}

\newcommand*{\shortref}[2][]{\hyperref[{#2}]{\autoref*{#2}#1}}

\newcommand*{\shorterref}[2][]{\hyperref[{#2}]{\ref*{#2}#1}}

% CODE STYLING -----------------------------------------------------

\definecolor{codegreen}{rgb}{0,0.6,0}
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{codepurple}{rgb}{0.58,0,0.82}
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}

\lstdefinestyle{mystyle}{
    backgroundcolor=\color{backcolour},
    commentstyle=\color{codegreen},
    keywordstyle=\color{magenta},
    numberstyle=\tiny\color{codegray},
    stringstyle=\color{codepurple},
    basicstyle=\ttfamily\footnotesize,
    breakatwhitespace=false,
    breaklines=true,
    keepspaces=true,
    numbers=left,
    numbersep=5pt,
    showspaces=false,
    showstringspaces=false,
    showtabs=false
}

\lstset{style=mystyle}

%\usepackage{fancyhdr}
%\pagestyle{fancy}
\setlength{\headheight}{14.49998pt}

% ------------------------------------------------------------------

\begin{document}

\begin{titlepage}
    \begin{center}
        \vspace*{-.2cm}
        UNIVERSIDADE DE LISBOA
        
        Faculdade de Medicina
        
        \includegraphics[width=0.25\textwidth]{images/logo/ulisboa-only}
        
        \vspace{1.8cm}
        Developing Web Apps for Analyses of Transcriptomes

        \vspace{1.1cm}        
%        \vspace{0.5cm}
%        Thesis Subtitle
            
        \vspace{0.9cm}            
        Nuno Daniel Saraiva Agostinho
    \end{center}

    \vspace{0.9cm}
    \begin{tabbing}
        Orientadores: \= Prof. Doutor Nuno Luís Barbosa Morais \\
                      \> Prof. Doutor Sérgio Alexandre Fernandes de Almeida \\
    \end{tabbing}
    
    \vspace{1.8cm}
    \begin{center}        
        Tese especialmente elaborada para obtenção do grau de Doutor em\\
        Ciências Biomédicas, Ramo da Biologia Computacional
            
        \vfill
        2023
        \vspace{.7cm}
    \end{center}
\end{titlepage}

\begin{titlepage}
    \begin{center}
        \vspace*{-.2cm}
        UNIVERSIDADE DE LISBOA
        
        Faculdade de Medicina
        
        \includegraphics[width=0.25\textwidth]{images/logo/ulisboa-only}

        Developing Web Apps for Analyses of Transcriptomes
            
        Nuno Daniel Saraiva Agostinho
    \end{center}

    \begin{tabbing}
        Orientadores: \= Prof. Doutor Nuno Luís Barbosa Morais \\
                      \> Prof. Doutor Sérgio Alexandre Fernandes de Almeida
    \end{tabbing}
    
    \begin{center}
        Tese especialmente elaborada para obtenção do grau de Doutor em\\
        Ciências Biomédicas, Ramo da Biologia Computacional
    \end{center}

    \noindent\begin{tabular}{@{} l p{13.1cm} }
        \textbf{Júri:} \\
        
        Presidente: & Doutor Mário Nuno Ramos de Almeida Ramirez, Professor Associado com Agregação e Vice-Presidente do Conselho Científico da Faculdade de Medicina da Universidade de Lisboa \\
        
        Vogais: & PhD Manuel Irimia, Group Leader e Research Professor do Centre for Genomic Regulation (Espanha) \\

        & Doutora Ana Rita Fialho Grosso, Professora Auxiliar da Faculdade de Ciências e Tecnologia da Universidade Nova de Lisboa \\

        & PhD Alejandro Reyes, Especialista de Reconhecido Mérito e Competência, Principal Scientist do Novartis Institutes for BioMedical Research (Switzerland) \\

        & Doutora Margarida Henriques da Gama Carvalho, Professora Auxiliar da Faculdade de Ciências da Universidade de Lisboa \\

        & Doutor Nuno Luis Barbosa Morais, Professor Associado Convidado da Faculdade de Medicina da Universidade de Lisboa (Orientador)

    \end{tabular}

    \begin{center}
        Instituição Financiadora:
        Fundação para a Ciência e Tecnologia (SFRH/BD/131312/2017 e COVID/BD/151928/2021)
        
        2023
    \end{center}
\end{titlepage}

\begin{titlepage}
    \begin{center}
        \vspace*{\fill}
        As opiniões expressas nesta publicação são\\
        da exclusiva responsabilidade do seu autor. \\
        
        \vspace*{2cm}
        
        \begingroup
            \fontsize{14pt}{12pt}\selectfont
            \textbf{A impressão desta tese foi aprovada pelo Conselho Científico da Faculdade de Medicina de Lisboa em reunião de 1 de fevereiro de 2023.}
        \endgroup
    \end{center}
\end{titlepage}


\pagenumbering{roman}
\tableofcontents

\selectlanguage{portuguese}
\input{chapters/resumo}

\selectlanguage{british}
\input{chapters/summary}

\input{chapters/acknowledgements}

%\setlength\epigraphwidth{5cm}
%\setlength\epigraphrule{0pt}
%\epigraph{\begin{CJK}{UTF8}{min}未練なく散も桜はさくら哉\end{CJK}
%I write, erase, rewrite
%Erase again, and then
%A poppy blooms.}{--- \textup{Hokushi Tachibana}}

\listoffigures
\addcontentsline{toc}{chapter}{List of Figures}

\listoftables
\addcontentsline{toc}{chapter}{List of Tables}

\input{chapters/preface}
\input{chapters/introduction}

\input{chapters/objectives}
%\input{chapters/methods}

\input{chapters/psichomics}
\input{chapters/cTRAP}
\input{chapters/appServer}

\input{chapters/discussion}

% citations using BibTeX
\bibliographystyle{plos2015}
\twocolumn
{\footnotesize
\bibliography{refs}}
\onecolumn

%\appendix
%\chapter{Appendix}
%\input{chapters/appendix}

\end{document}
