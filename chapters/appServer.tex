...
In our app server, we use Celery to run background tasks, alongside Flower to manage Celery jobs via its graphical interface and HTTP API\footnote{More information in \fullref{subsec:background-tasks}.}. We also need to use Redis as a database broker to communicate between the two Docker containers. To run Celery in Docker images, the Celery Python package should be installed with the programs that will be run in Celery jobs.

Currently, cTRAP is the only web app in our app server to use background tasks. As Celery is easy to install, the Docker image of Celery is custom-built based on the official cTRAP Docker image from Docker Hub (\dockerlink{nunoagostinho/ctrap}) with the Celery app installed on top.

To add a new Celery worker, a Dockerfile needs to be created based on a Docker image for the new app to run in the background. A separate service needs to be included in the Nginx configuration, similar to the code snippet shown in \shortref{lst:celery-worker}.

\begin{lstlisting}[caption=Template for a new Celery worker service in \texttt{docker-compose.yml}.,language=XML,label={lst:celery-worker}]{celery-worker}
  celery-ctrap:
